...
model {
  ...
  target += exponential_lpdf(nu | 1.0);
  ...
  for (n in 1 : N) {
    ...
    if (num_obs == J) {
      target += multi_student_t_lpdf(Y[n] | nu, eta[n], Sigma);
    } else if (num_obs > 0) {
      array[num_obs] int idx_obs = which_equal(R[n], 0);
      matrix[num_obs, num_obs] S = sub_matrix(Sigma, idx_obs);
      target += multi_student_t_lpdf(Y[n, idx_obs] | nu, eta[n, idx_obs], S);
    }
  }
}
generated quantities {
  ...
  for (n in 1 : N) {
    ...
    int num_obs = num_matches(R[n], 0);
    if (num_obs == J) {
      log_lik[n] = multi_student_t_lpdf(Y[n] | nu, eta[n], Sigma);
    } else if (num_obs > 0) {
      array[num_obs] int idx_obs = which_equal(R[n], 0);
      matrix[num_obs, num_obs] S = sub_matrix(Sigma, idx_obs);
      log_lik[n] = multi_student_t_lpdf(Y[n, idx_obs] | nu, eta[n, idx_obs], S);
    }
    Ypred[n] = to_row_vector(multi_student_t_rng(nu, eta[n], Sigma));
  }
}
